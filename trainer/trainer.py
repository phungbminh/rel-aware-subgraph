...
def run_training(
    ...
    is_main_process = (not use_ddp) or (local_rank == 0)
    ...
    if use_ddp:
        ...
        if torch.cuda.device_count() > 1 and str(device).startswith("cuda"):
            model = torch.nn.DataParallel(model)
            if is_main_process:
                print(f"[DEBUG][run_training] Using DataParallel on {torch.cuda.device_count()} GPUs!")
        model = model.to(device)
        if is_main_process and is_debug:
            print(f"[DEBUG][run_training] Using single-GPU (or DataParallel if >1 GPU).")
    ...
    if is_debug and is_main_process:
        print(
            f"[DEBUG][run_training] DataLoader created. Pin memory: {pin_memory}, num_workers: {num_workers}, DDP: {use_ddp}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_mrr > best_mrr:
            ...
            no_improve += 1
            if no_improve >= patience and is_main_process:
                msg = f"Early stopping after {patience} epochs without improvement"
                print(msg) if logger is None else logger.info(msg)
                break

    # --- Load best model checkpoint ---
    if checkpoint_path and os.path.exists(checkpoint_path) and is_main_process:
        state = torch.load(checkpoint_path, map_location=device)
        model.load_state_dict(state['state_dict'])
        if is_debug:
            print("[DEBUG][run_training] Loaded best model for testing")
    ...
    if is_main_process:
        ...
        return None  # Các process phụ không trả về gì
