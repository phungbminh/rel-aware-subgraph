...
# def deserialize(data):
#     """
#     Convert bytes from LMDB back into subgraph data dict.
#     Handles both raw pickle payload and optional 4-byte length header.
#     Returns None if data is not a pickled subgraph.
#     """
#     # Try direct unpickle
#     try:
#         data_tuple = pickle.loads(data)
#     except (pickle.UnpicklingError, EOFError):
#         # Strip possible 4-byte little-endian length header
#         header_size = struct.calcsize('<I')
#         if len(data) <= header_size:
#             return None
#         try:
#             data_tuple = pickle.loads(data[header_size:])
#         except (pickle.UnpicklingError, EOFError):
#             # Not a serialized subgraph
#             return None
#     # Map back to dict
#     keys = ('nodes', 'r_label', 'g_label', 'n_label')
#     if not isinstance(data_tuple, tuple) or len(data_tuple) != len(keys):
#         return None
#     return dict(zip(keys, data_tuple))
